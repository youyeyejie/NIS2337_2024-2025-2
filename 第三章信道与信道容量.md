...
- [第3章 信道与信道容量](#第3章-信道与信道容量)
    - [3.1 信道的数学模型](#31-信道的数学模型)
        ...
        - [无干扰信道（无噪声）](#无干扰信道无噪声)
        - [有干扰无记忆信道](#有干扰无记忆信道)
            - [1. 二进制离散对称信道（Binary Symmetric Channel，BSC）](#1-二进制离散对称信道binary-symmetric-channelbsc)
            - [2. 离散无记忆信道（Discrete Memoryless Channel，DMC）](#2-离散无记忆信道discrete-memoryless-channeldmc)
            - [3. 离散输入、连续输出信道](#3-离散输入连续输出信道)
            - [4. 波形信道](#4-波形信道)
    ...
    - [3.2 离散单个符号信道及其容量](#32-离散单个符号信道及其容量)
        - [无干扰离散信道](#无干扰离散信道)
            - [1. 无噪无损信道：n = m，X、Y一一对应](#1-无噪无损信道n--mxy一一对应)
            - [2. 无噪有损信道：n \> m，多个X对应一个Y](#2-无噪有损信道n--m多个x对应一个y)
            - [3. 有噪无损信道：n \< m，一个X对应多个Y](#3-有噪无损信道n--m一个x对应多个y)
        - [对称离散无记忆信道](#对称离散无记忆信道)
...
#### 1. 二进制离散对称信道（Binary Symmetric Channel，BSC）
- 输入\(X \in A = \{0, 1\}\)
- 输出\(Y \in B = \{0, 1\}\)
- \(
BSC\begin{cases}
...
#### 2. 离散无记忆信道（Discrete Memoryless Channel，DMC）
