...
- 对于任何满足 \(X \to Y \to \hat{X}\) 的估计量 \(\hat{X}\)，设 \(P_e = \Pr(X \neq \hat{X})\)，有：
    \[H(X|Y) \leq H(X|\hat{X}) \leq H(P_e)+ P_e \log|\mathcal{X}|\] 其中 \(\mathcal{X}\) 为 \(X\) 的取值空间。
- 上述不等式可以减弱为：
...
- 若信源是平稳的，即\(H(X_1)=H(X_2)=\cdots=H(X_L)=H(X)\)，则\[H(\vec{X}) = L H(X)\] 用 \(H_L(\vec{X})\) 表示长度为 \(L\) 的序列平均每个符号的熵，则
...
1. \(H(X_L|X^{L - 1})\) 是 \(L\) 的单调非增函数
    - 条件多的熵小于等于条件少的熵，平稳信源联合/条件概率时间推移不变
    - **证明**：\[
        \begin{align*}
...
3. \(H_L(\vec{X})\) 是 \(L\) 的单调非增函数
    - **证明**：\[
        \begin{align*}
        L H_L(\vec{X})&=H(X_1,X_2,\cdots,X_L)=H(X_1,X_2,\cdots,X_{L - 1})+H(X_L|X_1,X_2,\cdots,X_{L - 1})\\
        &=(L - 1)H_{L - 1}(\vec{X})+H(X_L|X_1,X_2,\cdots,X_{L - 1})\\
        &\leq (L - 1)H_{L - 1}(\vec{X})+H_L(\vec{X})
        \end{align*}
        \] \[
        \begin{align*}
        (L - 1)H_L(\vec{X})&\leq (L - 1)H_{L - 1}(\vec{X})\\
...
- **条件熵**：\[H_c(Y|X)=-\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}P_{X,Y}(x,y)\log P_Y(y|x)dxdy\]
...
- 对于限频 \(f_m\)，限时 \(t_B\) 的平稳随机过程，可用 \(L = 2f_mt_B\) 随机矢量表示，且有：
    \[
    \begin{align*}
...
3. **限平均功率最大熵定理**
    ...
    - 概率密度函数 \(P_X(x)=\frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{(x - m)^2}{2\sigma^2}}\)
    - 其中 \(m\) 为均值，\(\sigma^2\) 为方差，\(\sigma^2=\int x^2P_X(x)dx\) 。
    - 信息熵 \(H_c(X)\) 的计算过程如下：
    \[
    \begin{align*}
    H_c(X)&=-\int_{-\infty}^{+\infty}P_X(x)\log P_X(x)dx\\
    ...
    - **证明**：
        设 \(\varphi(x)\sim N(0,\sigma^2)\)，\(g(x)\) 满足 \(\int x^2g(x)dx=\sigma^2\) 。
        因为 \(D(g(x)\| \varphi(x))\geq0\)，即：
        \[
        \begin{align*}
        &D(g(x)\| \varphi(x))\geq0\\
